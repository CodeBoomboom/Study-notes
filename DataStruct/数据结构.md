...
快慢指针，一快一慢，注意慢指针啥时候停的这个条件

## 滑动窗口（双指针）

如果问题是求“连续子数组”往往用到滑动窗口
...
②设置startx、starty记录每一圈的起始位置

③设置一个offset偏移量，用来计算每一圈的起始位置，每走完一圈就+1，offset初始化为1，因为数组下标是从0开始的。以n=3第一圈的上为例，是从0—2的也就是从0—（3-1=2）的，通过j<n-offset,实现左闭右开，即左边在本条边计算，右边放到下一条边中计算；

⑤如果n是偶数的话那么正好循环loop圈，如果是奇数的话还要在最后再往矩阵中间的位置（mid = n/2)填最后一个数

方法2：模拟法

直接定义四条边的位置下标（上从0开始向下到n-1，左从0开始向右到n-1），通过while(count < num*num)模拟
...
一定要注意做链表类的很多题要加虚拟头结点，这样方便解决头结点的问题
...
熟用unordered_map、unordered_set

unordered_map是有键值对，unordered_set只有一个键，两者都不允许重复，如果插入重复元素会进行优化（只保留第一次插入的值，后序插入的都被优化）

insert()、count()、

可以直接用相同类型元素的迭代器初始化这两个容器 利用begin() end()



这两个题重点做：

注意 15.三数之和 和18.四数之和 这两个题，用哈希表不好去重，用双指针法做！！！固定一个，用两个指针分别从两端向中间遍历！（注意这两道题中的去重问题！要用nums[i-1]==nums[i]去，不能用nums[i]==nums[i+1]去!!）（还要注意这两题要排序！！）



# 字符串

1.反转字符串I：左右指针  用swap函数直接交换

2.反转字符串II：要先写一个像”翻转字符串I“一样的函数（或者直接用reserver库函数），然后再判断翻转

3.替换空格：可以直接用earse(int pos, int num)加insert(int pos, string s)

​				   也可以用双指针：要先resize扩容，然后ij分别指向扩容后和扩容前最后一个元素，然后依次向前遍历，遇到%20就加到s[j]向前的三个位置上，否则就s[j] = s[i]

​				  最简单的就是新建一个数组，依次遍历添加到新数组,但是这样空间复杂度高

4.翻转字符串中的单词：用栈的思想做 ，可以直接在判断中过滤空格，最后再加上空格就行。（若用代码随想录做法太麻烦了）（注意string也有push_back和pop_back)

5.左旋字符串：定义一个新字符串来存储  或者 不申请额外空间仅靠翻转（注意reverse函数翻转范围为[start, end) )

6.实现SubStr：KMP

7.重复的非空子串：KMP（在由重复子串组成的字符串中，最长相等前后缀不包含的子串就是最小重复字串，那么只要这个字符串的长度能够整除这个最小重复串的长度就说明是重复的非空子串）



## KMP算法

举例：文本串aabaabaaf   模式串aabaaf

如果是暴力搜索的话，外层遍历文本串，内层遍历模式串，从头开始一个一个字母遍历，当遍历到对应元素不相同（第一次遍历就是b与f不相同）的时候，文本串后移一位再遍历（文本串再从下一位开始与模式串从第一位比较）

但是KMP的思想是如果比较到当前文本串与模式串对应位不一样的时候，文本串不用动，模式串只需要跳到next数组的对应位置再匹配就行

这个next数组中的每一位存的是什么呢？是模式串中当前位之前的字符串的最长相等前后缀的长度。（由于数组下标从0开始，这个长度正好对应的就是相等前后缀中前缀的后一个位置）

那么怎么求这个next数组呢？ 用i来遍历模式串，j来表示i（包括i）之前的子串的最长相等前后缀的前缀位置，分为四步

1.i、j初始化，j应该从0开始，那么i应该从1开始

2.如果i、j对应的元素不相等,那么j要回退到上一个相等前缀的位置 while(j > 0 && s[i] != s[j]) j = next[j-1]

3.如果i、j对应元素相等，那么j++

4.更新next数组 next[i] = j，然后下一层循环，i++
...
二叉搜索树由于有次序，所以在迭代遍历的时候用指针就行了，不用再模拟栈

通过数组来构造二叉树，重点是找好“切割点”
...
### [#](https://www.programmercarl.com/二叉树总结篇.html#二叉树的遍历方式)二叉树的遍历方式

- 深度优先遍历

  - [二叉树：前中后序递归法 (opens new window)](https://programmercarl.com/二叉树的递归遍历.html)：递归三部曲初次亮相

    递归：一般来说要遍历整个二叉树就没有返回值，结束条件一般也是 node == NULL

  - [二叉树：前中后序迭代法（一） (opens new window)](https://programmercarl.com/二叉树的迭代遍历.html)：通过栈模拟递归

    迭代法的前中后都是通过stack模拟，后序（左右中）就是前序（中左右）的左右子树遍历顺序交换再反转，中序（左中右）需要再定义一个节点指针cur，先用这个指针接住root，再用这个指针一直遍历左，直到左NULL了然后再中再右

  - [二叉树：前中后序迭代法（二）统一风格(opens new window)](https://programmercarl.com/二叉树的统一迭代法.html)

    统一迭代也是通过stack遍历，只不过这次是通过在中节点后加入一个NULL节点作为记号，比如前序遍历（中左右），先push右入栈，然后是左，然后是中，中后面要加一个NULL节点，在循环时先判断当前要出栈节点是不是NULL节点，如果是NULL节点，则表示下一个节点是中节点，就要进行保存；如果不是NULL节点，则表明当前要出栈的节点不是中节点，先将其出栈，然后再按上述右左中NULL的顺序重新入栈。

- 广度优先遍历

  - [二叉树的层序遍历 (opens new window)](https://programmercarl.com/0102.二叉树的层序遍历.html)：通过队列模拟

    通过队列模拟，需要一个vector<int>数组和一个vector<vector<int>>数组，vector<vector<int>>数组用来保存最终答案，vector<int>数组用来保存每层的节点，还需要一个int size来记录每层的节点数目。先将root节点push进que，然后在循环中先记录size = que.size()，然后根据size的大小循环该层节点，放入vector<int>数组，并将该层节点的左右节点（如果有)放入que中，最后将vector<int>数组push_back到结果数组中.

### [#](https://www.programmercarl.com/二叉树总结篇.html#求二叉树的属性)求二叉树的属性

- 二叉树：是否对称(opens new window)
  - 递归：后序，比较的是根节点的左子树与右子树是不是相互翻转。左右子树翻转也就是说其外层（左子树左边右子树右边）相同，内层（左子树右边右子树左边）也相同，然后递归
...
- 二叉树：求最小深度(opens new window)
  ...
  - 迭代：层序遍历。在内层遍历时进行判断，一旦是叶子节点就return（return时一定是第一个叶子节点，也就是最小深度所在的节点）
- 二叉树：求有多少个节点(opens new window)
  - 递归：后序，通过递归函数的返回值计算节点数量。先找左子树数目，再找右子树数目，然后+1就是以当前节点为根节点的子树的节点数目
  - 迭代：层序遍历，记录节点数量
- 二叉树：是否平衡(opens new window)
  - 递归：后序，注意后序求高度和前序求深度，递归过程判断高度差。先求左子树高度，再求右子树高度，然后比较。abs函数：求绝对值
  - 迭代：效率很低，不推荐
- 二叉树：找所有路径(opens new window)
  - 递归：前序，方便让父节点指向子节点，涉及回溯处理根节点到叶子的所有路径。函数to_string()：转换为string
...
- 二叉树：求左叶子之和(opens new window)
  - 递归：后序，必须三层约束条件，才能判断是否是左叶子。先求左子树左叶子之和，再求右子树左叶子之和，然后再判断当前节点的左节点是否是左叶子，最后return
  - 迭代：直接模拟后序遍历
- 二叉树：求左下角的值(opens new window)
  - 递归：顺序无所谓，优先左孩子搜索，同时找深度最大的叶子节点。
  - 迭代：层序遍历找最后一行最左边
- 二叉树：求路径总和(opens new window)
  - 递归：顺序无所谓，递归函数返回值为bool类型是为了搜索一条边，没有返回值是搜索整棵树。遍历，做减法。
  - 迭代：栈里元素不仅要记录节点指针，还要记录从头结点到该节点的路径数值总和

### [#](https://www.programmercarl.com/二叉树总结篇.html#二叉树的修改与构造)二叉树的修改与构造

- 翻转二叉树(opens new window)
  - 递归：前序，交换左右孩子
  - 迭代：直接模拟前序遍历（深度、广度遍历都可以）
...
### [#](https://www.programmercarl.com/二叉树总结篇.html#求二叉搜索树的属性)求二叉搜索树的属性
...
- [求二叉搜索树的众数(opens new window)](https://programmercarl.com/0501.二叉搜索树中的众数.html)
  ...
  - 递归：反中序遍历（右中左），双指针操作累加
  - 迭代：模拟中序，逻辑相同

### [#](https://www.programmercarl.com/二叉树总结篇.html#二叉树公共祖先问题)二叉树公共祖先问题
...
### [#](https://www.programmercarl.com/二叉树总结篇.html#二叉搜索树的修改与构造)二叉搜索树的修改与构造

二叉树插入、删除、修建时递归最好有返回值，这样就可以直接利用返回值做重连接操作了，不然就需要另外定义一个pre

- 二叉搜索树中的插入操作(opens new window)
  - 递归：顺序无所谓，通过递归函数返回值添加节点
  - 迭代：按序遍历，需要记录插入父节点，这样才能做插入操作  （注意二叉搜索树有顺序，不用再像普通二叉树那样遍历）
- 二叉搜索树中的删除操作(opens new window)
  - 递归：前序，想清楚删除非叶子节点的情况
  - 迭代：有序遍历，较复杂  
  - 普通二叉树通用的删除操作：目标节点与目标节点右子树最左节点交换后删除
- 修剪二叉搜索树(opens new window)
  - 递归：前序，通过递归函数返回值删除节点
  - 迭代：有序遍历，较复杂
- 构造二叉搜索树(opens new window)
  - 递归：前序，数组中间节点分割。重点掌握好分割点
  - 迭代：较复杂，通过三个队列来模拟
...
### [#](https://www.programmercarl.com/0106.从中序与后序遍历序列构造二叉树.html#思路)思路
...
### [#](https://www.programmercarl.com/0106.从中序与后序遍历序列构造二叉树.html#c-完整代码)C++完整代码
...
### [#](https://www.programmercarl.com/0106.从中序与后序遍历序列构造二叉树.html#c-优化版本)C++优化版本
...
### [#](https://www.programmercarl.com/0654.最大二叉树.html#思路)思路
...
### [#](https://www.programmercarl.com/0654.最大二叉树.html#拓展)拓展
...
### [#](https://www.programmercarl.com/0654.最大二叉树.html#总结)总结

这道题目其实和 [二叉树：构造二叉树登场！ (opens new window)](https://programmercarl.com/0106.从中序与后序遍历序列构造二叉树.html)是一个思路，比[二叉树：构造二叉树登场！ (opens new window)](https://programmercarl.com/0106.从中序与后序遍历序列构造二叉树.html)还简单一些。
...
https://www.bilibili.com/video/BV12Z4y15721?spm_id_from=333.1007.top_right_bar_window_history.content.click

  //先求出从根节点到达p、q节点的路径

  //再求出两条路径上最后一个相同的节点，就是所求最近公共祖先

  //如题目示例：p的路径：35 q路径：31，最后一个相同节点是3
...
2. 如果找到一个节点，发现其左子树出现结点p，右子树出现节点q，或者 左子树出现结点q，右子树出现节点p，那么该节点就是节点p和q的最近公共祖先。**(p、q不可能都在一个节点的左子树或右子树，因为这样的话它们的最近公共祖先肯定是它俩中的一个)**
3. 如果p或者q本身就是最近公共祖先呢？其实只需要找到一个节点是p或者q的时候，直接返回当前节点，无需继续递归子树。如果接下来的遍历中找到了后继节点满足第一种情况（即后找到的p/q节点与先找到的q/p节点分属于同一节点的左右子树）则修改返回值为后继节点，否则，继续返回已找到的节点即可。
...
public:
    TreeNode* trimBST(TreeNode* root, int low, int high) {
        ...
        if (root->val < low) {//若root->val < low则表明root的左子树全小于low，从其右子树中找第一个大于low小于high的节点
            TreeNode* right = trimBST(root->right, low, high); // 寻找符合区间[low, high]的节点，会递归遍历右子树所有节点，选出所有符合的节点
            return right;//此时找到的right就是第一个符合[L, R]的节点，返回后就接入了上一个节点的左/右节点
        }
        if (root->val > high) {
            TreeNode* left = trimBST(root->left, low, high); // 寻找符合区间[low, high]的节点，会递归遍历右子树所有节点，选出所有符合的节点
            return left;//此时找到的left就是第一个符合[L, R]的节点，返回后就接入了上一个节点的左/右节点
        }
        
        //到这一步说明root在范围内，则分别找其左右子树中符合条件的节点，接到其左右节点上
        root->left = trimBST(root->left, low, high); // root->left接入符合条件的左孩子
        root->right = trimBST(root->right, low, high); // root->right接入符合条件的右孩子
        return root;//最后返回root，将其接到上一级节点上
    }
};
```
...
### [#](https://www.programmercarl.com/回溯总结.html#组合总和)组合总和

#### [#](https://www.programmercarl.com/回溯总结.html#组合总和-一)组合总和（一）
...
#### [#](https://www.programmercarl.com/回溯总结.html#组合总和-二)组合总和（二）
...
#### [#](https://www.programmercarl.com/回溯总结.html#组合总和-三)组合总和（三）
...
### [#](https://www.programmercarl.com/回溯总结.html#多个集合求组合)多个集合求组合
...
### [#](https://www.programmercarl.com/回溯总结.html#切割问题)切割问题
...
### [#](https://www.programmercarl.com/回溯总结.html#子集问题)子集问题

#### [#](https://www.programmercarl.com/回溯总结.html#子集问题-一)子集问题（一）
...
#### [#](https://www.programmercarl.com/回溯总结.html#子集问题-二)子集问题（二）
...
### [#](https://www.programmercarl.com/回溯总结.html#递增子序列)递增子序列
...
### [#](https://www.programmercarl.com/回溯总结.html#排列问题)排列问题

#### [#](https://www.programmercarl.com/回溯总结.html#排列问题-一)排列问题（一）
...
#### [#](https://www.programmercarl.com/回溯总结.html#排列问题-二)排列问题（二）
...
### [#](https://www.programmercarl.com/回溯总结.html#去重问题)去重问题
...
### [#](https://www.programmercarl.com/回溯总结.html#重新安排行程-图论额外拓展)重新安排行程（图论额外拓展）
...
![img](https://img-blog.csdnimg.cn/2020111518065555.png)
...
### [#](https://www.programmercarl.com/回溯总结.html#棋盘问题)棋盘问题

#### [#](https://www.programmercarl.com/回溯总结.html#n皇后问题)N皇后问题
...
相信看完本篇[回溯算法：N皇后问题 (opens new window)](https://programmercarl.com/0051.N皇后.html)也没那么难了，传说已经不是传说了，哈哈。

#### [#](https://www.programmercarl.com/回溯总结.html#解数独问题)解数独问题
...
1.大饼干优先满足胃口大的：需要对胃口数组从后向前遍历，当前饼干满足了当前胃口饼干索引才减一，不满足那么胃口索引减一

2.小饼干优先满足胃口小的：需要对饼干数组从前往后遍历，当前饼干满足了当前胃口胃口索引才加一，不满足那么饼干索引加一



## 摆动序列

忽略掉单调坡度上的点计算峰值数目。注意result要初始化为1！这样是为了考虑数组最左面/最右面的特殊情况，结果初始化为1（也可以认为最终得到的序列值总是比峰值区间多1）

注意preDiff判断中有=0这个判断，这是因为当计算第一个区间，要把计算的值存到preDiff中，不然就存不进去。



## 最大子数组和（最大子序列）

这个题用动规一定要注意dp[i]的定义：要定义为包括i之前的最大连续子数组和为dp[i]，主要要包括i！！否则没法做

根据dp定义，这个题的result并不是dp[i]，而是dp[i]中的最大值



贪心则简单了：当前“连续和”为负数的时候立刻放弃，从下一个元素重新计算“连续和”，因为负数加上下一个元素 “连续和”只会越来越小。

用一个count一直从第一个元素累加，并每次都比较更新result，一旦count小于0了直接将count置0
...
求爬上楼梯的方法：dp[i] = dp[i-1] + dp[i-2]

求最小花费（默认往上爬的时候才花费）：dp[i] = min(dp[i-1]+cost[i-1], dp[i-1]+cost[i-1])



### 不同路径

无障碍求机器人到达右下角共有多少不同的路径：

​	dp[i] [j] = dp[i-1] [j] +dp[i] [j-1]  注意初始化：dp[i] [0] = dp[0] [j] =1

有障碍求机器人到达右下角共有多少不同的路径：

​    dp[i] [j] = dp[i-1] [j] +dp[i] [j-1] 若dp[i] [j]是障碍物则dp[i] [j] = 0

​	初始化的时候也要注意为障碍物的话，之后的格子都无法到达了



### 整数拆分

注意怎么找出的递推关系：i可以拆分为j和i-j，i-j不再拆分。或者i-j继续拆分，比较两个取最大值。然后因为j是从1到i-1的，要比较对于所有的j中dp[i]的最大值
...
重点是要搞明白递推公式：要遍历从1到i的每个元素j，将以每个j元素为树根的所有二叉树种数累加起来，其中以j为根节点的种数就是 以j-1为根节点的种数 加上 以i-j为根节点的种数。也就是：

dp[i] += dp[以j为头结点左子树节点数量] * dp[以j为头结点右子树节点数量]

```cpp
//1、确定dp数组及其下标含义：dp[i]：代表由i个节点组成且节点值从1到i互不相同的二叉树的种数
//2、确定递推公式：遍历1~i的每个数字j，将该数字j作为树根，那么
//      dp[i] += dp[以j为头结点左子树节点数量] * dp[以j为头结点右子树节点数量]
//      1....j-1,j,....i 左边就是j-1 右边就是i-j
//      即  dp[i] += dp[j-1]*dp[i-j]
/*  如：dp[3]，就是 元素1为头结点搜索树的数量+元素2为头结点搜索树的数量+元素3为头结点搜索树的数量
                元素1为头结点搜索树的数量 = 右子树有2个元素的搜索树数量 * 左子树有0个元素的搜索树数量
                元素2为头结点搜索树的数量 = 右子树有1个元素的搜索树数量 * 左子树有1个元素的搜索树数量
                元素3为头结点搜索树的数量 = 右子树有0个元素的搜索树数量 * 左子树有2个元素的搜索树数量
                即dp[3] = dp[2] * dp[0] + dp[1] * dp[1] + dp[0] * dp[2]   */   
//3、初始化：空节点也是一个二叉搜索树，dp[0]=1
//4、递推顺序：节点数为i的状态是依靠i之前节点数的状态，所以从前向后
```
...
**做leetcode题目的时候，重点就是将问题如何转化为01背包问题**
...
```cpp
// 初始化 dp  weight.size()就是物品的个数 bagweight+1是因为数组下标从0开始
//全部初始化为0其实就是省略了将dp[i][0]初始化为0和将j<weight[0]情况下的dp[0][j]初始化为0的情况
...
![image-20220810150950432](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20220810150950432.png)

![image-20220810150959703](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20220810150959703.png)
...
![image-20220810154742210](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20220810154742210.png)
...
![image-20220812155925191](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20220812155925191.png)

#### 一维dp数组（滚动数组）完全背包个人总结

![image-20220825171445367](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20220825171445367.png)
...
* **数组上连续元素二选一，所能得到的最大价值**

  1、dp[i]表示在0到i的房屋中考虑所能得到的最大价值

  2、递推公式：考虑第i个房子：dp[i] = dp[i-2]+nums[i] 

  ​					     不考虑第i个房子 dp[i] = dp[i-1]

  ​						 所以 dp[i] = max(dp[i-2]+nums[i], dp[i-1])

  3、初始化：由递推公式得dp[i]最初是由dp[0]和dp[1]得到的

  ​					dp[0] = nums[0]；dp[1] = max(nums[0], nums[1])

  4、遍历顺序：从前向后

* **成环之后连续元素二选一，所能得到的最大价值**

  分为两种情况：考虑首元素不考虑尾元素、考虑尾元素不考虑首元素。只要根据数组上“连续元素二选一”分别计算这两种情况下的最大价值，然后比较得出最终的结果就行。

* **在树上连续元素二选一，所能得到的最大价值**

  这个可以用递归来做，分为偷root节点（偷到的价值 = root节点的val +liangge 孙子节点的val ，也就是说跳过子节点）和不偷root节点（偷到的价值 = 两个子节点的val)两种，但是只用递归会超时，因为你在偷root节点和不偷root节点递归的过程中进行了大量重复计算，需要进行记忆化递归，使用一个`unordered_map<TreeNode* ,int> flag;`数组来记忆每个计算过的节点的最大价值，在递归一开始判断是否已经计算过了，如果计算过了就直接返回，没计算过的才进行下一步计算

  也可以用动规结合二叉树遍历来做。二叉树的遍历必须用后序遍历，因为我们需要通过递归的子节点的返回值来计算父节点，只有后序遍历才能保证先计算子节点后计算父节点。递归的返回值返回一个大小为2的vector<int> 数组, vector[0]表示不偷当前节点得到的最大价值，vector[1]表示偷当前节点得到的最大价值，这个也就相当于每个节点的dp数组表达式的含义；当遍历到叶子节点时，应该返回{0,0}，偷与不偷都是0，这也相当于dp数组的初始化；递归的时候首先得到左右子节点的vector数组（dp），偷当前节点的时候`int val1 = cur->val + left[0] + right[0];`表示当前节点的val加上不偷左右子节点的val，不偷当前节点的时候`int val2 = max(left[0], left[1]) + max(right[0], right[1]);`表示左右子节点的最大价值相加。

### 买卖股票问题

#### 1. **买卖股票的最佳时机（只能买卖一次，求最大收益）**

可以用暴力解法（超时）、贪心（股票就买卖一次，那么贪心的想法很自然就是取最左最小值，取最右最大值，那么得到的差值就是最大利润。）

动态规划：

1、dp[i] [0]:表示第i天持有股票所得最多现金  dp[i] [1]：第i天不持有股票所得现金

2、递推公式：

​	  dp[i] [0]即第i天持有股票所得现金，可以由两种情况得到，求两种情况最大值：①第i-1天就已经持有股票了，此时dp[i] [0]=dp[i-1] [0]  ②第i-1天没有持有股票，也就是说第i天才买股票，则此时现金为 dp[i] [0] = -prices[i] （第一天买股票，之前现金都是0）

​	  dp[i] [1]即第i天不持有股票所得现金，可以由两种情况得到，求两种情况最大值：①第i-1天就不持有股票，此时dp[i] [1]=dp[i-1] [1]	   ②第i-1天持有股票但是第i天卖了，那么所得现金就是按照今天股票佳价格卖出后所得现金加上之前前一天持有股票的现金，即dp[i] [1]=dp[i-1] [0] + prices[i]

3、初始化：

​	由递推公式得依赖dp[0] [0]和dp[0] [1] ，第0天持有股票dp[0] [0] = -price[0]；第0天不持有股票dp[0] [1] = 0;

4、遍历顺序：从前向后

#### 2. **买卖股票的最佳时机II （可以买卖多次）**

这个题可以用暴力（超时）、贪心（比较好想，只要求出每天的收益，将正收益相加就行）

动态规划：

除了递推公式之外其他的都与“只能买卖一次”相同

递推公式：

​		dp[i] [0]第i天持有股票的最大现金，可以由两种情况得来，求两种情况最大值：①第i-1天就持有股票，那么此时dp[i] [0] = dp[i-1] [0]  ②第i-1天不持有股票，第i天又买了股票，dp[i] [0] = dp[i-1] [1] - prices[i] (**注意：因为本题可以买卖多次，所以第i次买股票之前可能已经买卖过股票了，所以要加上第i-1次不持有股票的最大现金；而“只能买卖一次”则能保证是第一次买股票，所以现金直接-prices[i]就行**)

​		 dp[i] [1]即第i天不持有股票所得现金，可以由两种情况得到，求两种情况最大值：①第i-1天就不持有股票，此时dp[i] [1]=dp[i-1] [1]	   ②第i-1天持有股票但是第i天卖了，那么所得现金就是按照今天股票佳价格卖出后所得现金加上之前前一天持有股票的现金，即dp[i] [1]=dp[i-1] [0] + prices[i]

#### 3. **买卖股票的最佳时机III（最多可以完成两笔交易，且两笔交易不能同时进行）**
...
​	  ①第i天没有操作，那么就等于dp[i-1] [0]

​	  ②第i天第一次买入股票，dp[i] [1]可以由两种情况得到，取其最大值：第一种就是第i天第一次买入dp[i] [1] = dp[i-1] [0] - prices[i];第二种是延续第i-1天第一次买入的状态dp[i] [1] = dp[i-1] [1]

​		③第i天第一次卖出股票，dp[i] [2]可以由两种情况得到，取其最大值：第一种就是第i天第一次卖出dp[i] [2] = dp[i-1] [1] + prices[i];第二种是延续第i-1天第一次卖出的状态dp[i] [2] = dp[i-1] [2]

​		④第i天第二次买入股票，dp[i] [3]可以由两种情况得到，取最大值：第一种就是第i天第二次买入股票dp[i] [3] = dp[i-1] [2] - prices[i]; 第二种就是第i天延续第i-1天第二次买入股票的状态dp[i] [3] = dp[i-1] [3]

​		⑤第i天第二次卖出股票，dp[i] [4]可以由两种情况得到，取最大值：第一种就是第i天第二次卖出股票dp[i] [4] = dp[i-1] [3] - prices[i]; 第二种就是第i天延续第i-1天第二次买入股票的状态dp[i] [3] = dp[i-1] [3]
...
​		③第0天第一次卖出，即第0天第一次买入并卖出：dp[0] [2] = 0

​		④第0天第二次买入，即第0天买入并卖出后再买入：dp[0] [3] = -prices[0]

​		⑤第0天第二次卖出，即第0天买入并卖出后再买入并卖出：dp[0] [4] = 0

4、遍历顺序 从前向后

#### 4. **买卖股票的最佳时机IV（最多可以买卖k次)**
...
​	  一天一共就有k+1个状态：
  	...
  	0没有操作    1第一次买入   2第一次卖出   ... 第k次买入  第k次卖出

​	 dp[i] [j]中 i表示第i天，j为 [0 - k] k+1个状态，dp[i] [j]表示第i天状态j所剩最大现金

2、递推公式：

​		循环的时候需要两重循环，i负责循环天数，j则循环每天的状态（第几次买卖），首先无论哪天无操作均为0，所以就不用加到循环了，只要dp[i] [0]初始化为0就行。要注意的是，因为每次操作均有买入卖出两种操作，再加上0无操作，所以j要初始化为2*k+1，j要从0开始循环，这样才能保证j+1，j+2是从数组第1,2个元素开始的，且每次循环要+2(越过每次的买入卖出操作)

​	  ①第i天第j次买入(j+1)，等于第i-1天第j次买入 与 第i-1天第j-1次卖出(注意这里第j-1次卖出是 j-2+2=j)再在第i天买入 的最大值：dp[i] [j+1] = max(dp[i-1] [j+1], dp[i-1] [j]-prices[i])

​	  ②第i天第j次卖出(j+2)，等于第i-1天第j次卖出 与 第i-1天第j次买入(注意这里第j次买入是 j+1)再在第i天卖出 的最大值：dp[i] [j+2] = max(dp[i-1] [j+2], dp[i-1] [j-1])

3、初始化

​		①第0天无操作：dp[0] [0] = 0;

​		②第0天第j次买入：dp[0] [j] = -prices[0]
...
![img](https://img-blog.csdnimg.cn/518d5baaf33f4b2698064f8efb42edbf.png)



1、dp[i] [j]:第i天状态为j，所剩的最多现金为dp[i] [j] 

​	状态：状态1：买入股票状态（今天买入股票，或者之前买入股票但没有操作）

​				状态2：保持卖出股票状态（之前就是卖出了股票并度过了冷冻期）
...
​		状态1：买入股票状态可以由三种情况得来：①前一天就是买入状态②前一天是状态2保持卖出股票状态③前一天是冷冻期状态

​					dp[i] [0] = max(dp[i-1] [0], max(dp[i-1] [3], dp[i-1] [1])-prices[i]);

​		状态2：卖出股票保持状态可以由两种情况得来：①前一天就是卖出状态②前一天是冷冻期

​					dp[i] [1] = max(dp[i-1] [1], dp[i-1] [3]);

​		状态3：卖出股票状态可以由一种情况得来：前一天是买入股票状态

​					dp[i] [2] = dp[i-1] [0]+prices[i];

​		状态4：冷冻期状态可以由一种情况得来：前一天是卖出股票状态

​					dp[i] [2] = dp[i-1] [0]+prices[i];

3、初始化 第0天

​	状态1初始化为-prices[0]，其他都初始化为0
...
与2.买卖股票的最佳时机II （可以买卖多次）的区别就是要减一个手续费，这个手续费是在刚买入/卖出的时候减去是无所谓的
